...
public class GuiSpaceEye extends GuiContainerBase<FakeTileEntity> {
    ...
    public void initGui() {
        ...
        this.elements.clear();
        List<Spacecraft> satellites = SpaceData.spacecrafts;
        for (int i = 0; i < satellites.size(); i++) {
            elements.add(new SpacecraftElement(satellites.get(i), this.guiLeft + 7, this.guiTop + i * 7 + 7));
        }
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        if(this.activeElement != -1){
            if(GuiUtil.isMouseInRange(mouseX, mouseY, this.guiLeft, this.guiTop, 146, 96, 26, 18)){
                ...
                IElement element = this.elements.get(this.activeElement);
                if(element instanceof SpacecraftElement){
                    SpaceData.spacecrafts.remove(((SpacecraftElement) element).spacecraft);
                }
                initGui();
        ...
        if(GuiUtil.isMouseInRange(mouseX, mouseY, this.guiLeft, this.guiTop, 119, 96, 26, 18)){
            SpaceData.spacecrafts.add(new Satellite(String.valueOf(mouseX + mouseY), new SpacePosition(Planet.EARTH), SatTypes.COMMUNICATION));
            initGui();
        }
    }
    ...
    public class SpacecraftElement extends IElement{

        public Spacecraft spacecraft;

        public SpacecraftElement(Spacecraft spacecraft, int x, int y){
            super(x, y);
        ...
        public String getFirstLine(){
            return TextFormatting.BLACK + this.spacecraft.getName();
        }

        @Nullable
        @Override
        public String getSecondLine() {
            return TextFormatting.DARK_GRAY + this.spacecraft.getPosition().getClosestLocationString("Location: ");
        }

        @Nonnull
        @Override
        public List<String> getDescription() {
            return this.spacecraft.getAttributes();
        }
    }
